...
export const getAllMessages = asyncHandler(async (req, res) => {
    ...
    // if (!selectedChat.participants?.includes(req.auth?._id)) {
...
export const sendMessage = asyncHandler(async (req, res) => {
    ...
    const message = await ChatMessage.create({
      sender: new mongoose.Types.ObjectId(req.auth._id),
      content: content || "",
      chat: new mongoose.Types.ObjectId(chatId),
    ...
    chat.participants.forEach((participantObjectId) => {
      ...
      if (participantObjectId.toString() === req.auth._id.toString()) return;
...
//     if (message.sender.toString() !== req.auth._id.toString()) {
//         throw new ApiError(
//             401,
